...
::: info
The `cmu.config.ts` generated by `cmu create` does **not** contain a `network.explorerUrl` key, so `cmu explorer open` falls back to the default until the key is added by hand:
...
Only `cmu.config.ts` is consulted here. Unlike `cmu compile`, this command does not fall back to `cmu.config.js`.
:::
